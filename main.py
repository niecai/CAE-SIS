...
def img2share(img, k, n, share_road, direc_generation=False, encryption_method=None,
    ...
    feature = AE.train(data[np.newaxis, :, :, np.newaxis],
                       my_epochs=100000, pre_train_model=pre_train_model)
...
data = ['share_result/{}_share0'.format(img_name),
        'share_result/{}_share1'.format(img_name),
        'share_result/{}_share2'.format(img_name),
        'share_result/{}_share3'.format(img_name)]
ch = []
num = 0
    ...
    if n == 4:
        if now != 0 and now != 4:
            share2img(ch, (128, 128), fea, encryption_method='AE',
                      output='reconsitution/{}/{}_{}.png'.format(now, img_name, num),
                      feature_map=fea_map, ks=ks, step=step)
            print('play {}_loss in {}'.format(num, str(ch)))
            num += 1
    else:
        choose(now, n + 1)
        ch.append(data[n])
